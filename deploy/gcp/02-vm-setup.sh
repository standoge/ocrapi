...
if [[ ! -f /etc/ocrapi.env ]]; then
  # 01-provision.sh publishes a fully filled env file to instance metadata
  # (key: ocrapi-env) so no hand edits are needed. Fall back to the template.
  META_ENV="$(curl -sf -H 'Metadata-Flavor: Google' \
    'http://metadata.google.internal/computeMetadata/v1/instance/attributes/ocrapi-env' || true)"
  if [[ -n "${META_ENV}" ]]; then
    echo "==> Installing /etc/ocrapi.env from instance metadata (generated by 01-provision.sh)..."
    printf '%s\n' "${META_ENV}" > /etc/ocrapi.env
    chmod 600 /etc/ocrapi.env
  else
